#!/usr/bin/env python3
"""
emit_phrases_pack.py

Reads phrases_canonical.json and generates playlist YAML files for the
Blues Phrases & Joe Pass Etudes collection.

Output: playlists/phrases_*.ztplay  (3 playlists)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "phrases_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Phrases -- {pl['title']}",
        f"# Auto-generated from phrases_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
