...
Reads cycles_canonical.json and generates playlist YAML files for the
Cycle Training collection.

Output: playlists/cycle_*.ztplay  (2 playlists)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "cycles_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Cycle Training -- {pl['title']}",
        f"# Auto-generated from cycles_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
