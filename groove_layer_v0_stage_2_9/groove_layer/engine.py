...
@dataclass
class GrooveStateV0:
    """Minimal state container for Stage 2.9.

    Adds Vector 09: density/tempo guardrails (no probe during correct_drift or micro_loop).
    """
...
def compute_groove_layer_control(payload: Dict[str, Any], state: Optional[GrooveStateV0] = None) -> Dict[str, Any]:
    ...
    Stage 2.9 adds Vector 09:
      - density probes are forbidden when tempo.policy == "correct_drift"
      - density probes are forbidden when last_loop_policy == "micro_loop"
    """
    if state is None:
        state = GrooveStateV0()
    ...
    # --- Vector 09: density/tempo guardrails ---
    # Probes are forbidden when tempo is correcting or we're in micro_loop recovery
    probe_forbidden = (
        tempo_control_v08["policy"] == "correct_drift" or
        state.last_loop_policy == "micro_loop"
    )

    # --- Probing + scheduling (Vector 06) with Vector 09 guardrails
    if (stable and
        (state.probe_cooldown_windows == 0) and
        (state.stable_windows >= 2) and
        (state.next_probe_in_windows == 0) and
        (not state.probe_active) and
        (not probe_forbidden)):  # Vector 09 guard
