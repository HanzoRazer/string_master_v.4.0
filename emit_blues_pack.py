...
Reads blues_canonical.json and generates playlist YAML files for the
Blues Turnarounds & Licks collection.

Output: playlists/blues_*.ztplay  (5 playlists)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "blues_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Blues -- {pl['title']}",
        f"# Auto-generated from blues_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
