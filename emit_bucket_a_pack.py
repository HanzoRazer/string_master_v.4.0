#!/usr/bin/env python3
"""
emit_bucket_a_pack.py

Reads bucket_a_canonical.json and generates playlist YAML files for the
Bucket A mixed etudes collection.

Output: playlists/bucket_a_*.ztplay  (8 playlists)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "bucket_a_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def yaml_str(s):
    ...
    if s is None:
        return "null"
    s = str(s)
    # Quote if starts with special chars, contains colons, or looks numeric
    if (s.startswith(("{", "[", "'", '"', "&", "*", "!", "|", ">", "%", "@", "`")) or
        ":" in s or
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Bucket A -- {pl['title']}",
        f"# Auto-generated from bucket_a_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
