...
Repeat behavior:
  - Rhythm is frozen
  - Motivic shape is preserved (hardcoded offsets)
...
# Canonical engine dataclass
from src.zt_band.midi_out import NoteEvent

# Your existing tag sampler + placement layer
from rock_tag_probability import Difficulty, RockStyle
...
# -----------------------------------------------------------------------------
# Hardcoded motivic seed (single source cell)
# -----------------------------------------------------------------------------
# 4 bars * 6 notes/bar = 24 offsets, in semitones, relative to the forced opening pitch.
#
# Design goals (blues-conversational):
#   - Bar 1: X -> chromatic upper neighbor -> X -> +3 (minor 3rd up) + small fill
#   - Bar 2: "dominant7 arpeggio vs minor pentatonic" answer gesture (transposable)
#   - Bar 3: dominant hover; melodic line avoids tonic pitch-class C (pc=0)
#   - Bar 4: resolution color without announcing tonic (leans to 3rd/6th colors)
#
# IMPORTANT: This is the only hardcoded melodic DNA. All variation comes from:
#   - approach tones insertion (repeat micro-variation)
#   - articulation sidecar tags
MOTIVIC_OFFSETS_SEMITONES: List[int] = [
    # Bar 1 (beats 0-4)
    0,  +1, 0,  +3, +2, +3,
    # Bar 2 (beats 4-8) — blues conversational answer: 9, root, b7, 5, 4, 5 (relative)
    +5, +3, +1, -2, -4, -2,
