#!/usr/bin/env python3
"""
emit_chromatic_triplets_pack.py

Reads chromatic_triplets_canonical.json and generates playlist YAML files for the
Chromatic Triplet Chains collection.

Output: playlists/chromatic_triplet_*.ztplay  (1 playlist)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "chromatic_triplets_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Chromatic Triplets -- {pl['title']}",
        f"# Auto-generated from chromatic_triplets_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
