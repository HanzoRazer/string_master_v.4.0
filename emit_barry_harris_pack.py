#!/usr/bin/env python3
"""
emit_barry_harris_pack.py

Reads barry_harris_canonical.json and generates playlist YAML files for the
Barry Harris Scales collection.

Output: playlists/barry_harris_*.ztplay  (1 playlist)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "barry_harris_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Barry Harris -- {pl['title']}",
        f"# Auto-generated from barry_harris_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
