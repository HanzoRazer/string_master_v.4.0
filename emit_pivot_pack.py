...
Reads pivot_canonical.json and generates playlist YAML files for the
Pivot Techniques collection.

Output: playlists/pivot_*.ztplay  (1 playlist)
"""
import json
import os
import sys
...
with open(os.path.join(ROOT, "pivot_canonical.json"), encoding="utf-8") as f:
    canon = json.load(f)
...
def emit_playlist(pl):
    ...
    lines = [
        f"# Pivot -- {pl['title']}",
        f"# Auto-generated from pivot_canonical.json",
        "",
        f"id: {pl['id']}",
        f'title: "{pl["title"]}"',
